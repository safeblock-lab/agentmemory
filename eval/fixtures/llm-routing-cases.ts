...
const SCENARIOS: readonly Scenario[] = [
  ...
  { id: "service-config", input: "Service agentmemory uses Ollama at http://127.0.0.1:11434/v1 with qwen3.5:4b. The primary model is accounts/fireworks/models/nemotron-lightning-3p5-30b-a3b.", terms: ["agentmemory", "qwen3.5:4b", "nemotron-lightning-3p5-30b-a3b"], critical: ["qwen3.5:4b", "nemotron-lightning-3p5-30b-a3b"], forbidden: ["DeepSeek"], classification: "architecture", relation: "agentmemory|uses|qwen3.5:4b" },
  ...
  { id: "procedure", input: "Procedure: run npm run build, copy dist to the installed runtime, then start AgentMemory-Watchdog. Keep qwen3.5:4b warm with keep_alive=-1.", terms: ["npm run build", "AgentMemory-Watchdog", "qwen3.5:4b", "keep_alive=-1"], critical: ["AgentMemory-Watchdog", "keep_alive=-1"], forbidden: ["restart Windows"], classification: "workflow", relation: "npm run build|precedes|AgentMemory-Watchdog" },
  ...
  { id: "multi-session", input: "Session A selected qwen3.5:4b for local structured tasks. Session B measured one malformed escape and one fact omission in summary. The decision after review: keep summary on Qwen with Nemotron fallback; do not promote it to primary-only.", terms: ["Session A", "qwen3.5:4b", "Session B", "Nemotron fallback"], critical: ["qwen3.5:4b", "Nemotron fallback"], forbidden: ["primary-only"], classification: "decision", relation: "summary|falls_back_to|Nemotron" },
  { id: "temporal-contradiction", input: "At 09:00 the watchdog reported Ollama unavailable. At 09:02 it recovered and model qwen3.5:4b was resident. The final state is healthy; preserve both timestamps and do not claim the outage persists.", terms: ["09:00", "Ollama unavailable", "09:02", "qwen3.5:4b", "healthy"], critical: ["09:02", "healthy"], forbidden: ["outage persists"], classification: "fact", relation: "qwen3.5:4b|resident_at|09:02" },
